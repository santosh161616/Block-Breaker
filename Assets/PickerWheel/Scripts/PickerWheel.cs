...
namespace EasyUI.PickerWheelUI
{
    public class PickerWheel : MonoBehaviour
    {
        ...
        [SerializeField] private List<Piece> _piecesList = new List<Piece>();

        // Events
        public static UnityAction onSpinStartEvent;
        public static UnityAction<Piece> onSpinEndEvent;
        ...
        private Vector2 pieceMinSize = new Vector2(56f, 95f);
        ...
        [SerializeField] private Transform _rewardIndicator;
        private void Start()
        {
        ...
        private void DrawPiece(int index)
        {
            ...
            Piece pieceTrns = InstantiatePiece();

            pieceTrns.Icon.sprite = piece.Icon;
            pieceTrns.Name.text = piece.Label;
            pieceTrns.Amount.text = piece.Amount.ToString();
            pieceTrns.value = piece.Amount;
            ...
            pieceTrns.transform.RotateAround(wheelPiecesParent.position, Vector3.back, (pieceAngle * index) /*+ halfPieceAngle*/);
            _piecesList.Add(pieceTrns);
        }
        ...
        public WheelPiece wheelpieceData;
        public void Spin()
        {
            if (!_isSpinning)
            {
                ...
                wheelpieceData = wheelPieces[index];

                if (wheelpieceData.Chance == 0 && nonZeroChancesIndices.Count != 0)
                {
                    index = nonZeroChancesIndices[Random.Range(0, nonZeroChancesIndices.Count)];
                    wheelpieceData = wheelPieces[index];
                }
                piece = _piecesList.Find(t => t.value == wheelpieceData.Amount);
                ...
                Vector2 initialPosition = _rewardIndicator.position;
                //Tween tween = _rewardIndicator.DOLocalMoveY(265f, 0.4f).SetLoops(-1, LoopType.Yoyo);
                //Tween tween = _rewardIndicator.DOLocalRotate(new Vector3(0, 0, 10), 0.4f).SetLoops(-1, LoopType.Yoyo);
                ...
                {
                    //tween.Kill();
                    //_rewardIndicator.position = initialPosition;
                    //_rewardIndicator.GetComponent<Rigidbody2D>().velocity = Vector2.zero;
        ...
        public void OnSpinEnd(UnityAction<Piece> action)
        {
